...
def track_lin_vel_xy_exp(
    ...
    # [修改点] 使用 quat_apply_inverse
    lin_vel_b = math_utils.quat_apply_inverse(asset.data.root_quat_w, asset.data.root_lin_vel_w)
    lin_vel_xy_b = lin_vel_b[:, :2]

    error = torch.sum(torch.square(commands - lin_vel_xy_b), dim=1)
    return torch.exp(-error / std ** 2)


# 2. 修正 track_ang_vel_z_exp
def track_ang_vel_z_exp(
    ...
    # [修改点] 使用 quat_apply_inverse
    ang_vel_b = math_utils.quat_apply_inverse(asset.data.root_quat_w, asset.data.root_ang_vel_w)
    ang_vel_z_b = ang_vel_b[:, 2]

    error = torch.square(commands - ang_vel_z_b)
    return torch.exp(-error / std ** 2)
...
def flat_orientation_l2(
    ...
    """惩罚非水平姿态 / Penalize non-flat base orientation.

    通过惩罚重力向量在机器人XY平面上的投影来实现。
    如果机器人完全水平，重力在XY平面的分量应为0。
    """
    asset: RigidObject = env.scene[asset_cfg.name]
    # projected_gravity_b 是重力向量在基座坐标系下的表示
    # 理想情况下应该是 [0, 0, -1]
    # 我们惩罚 x 和 y 分量
    return torch.sum(torch.square(asset.data.projected_gravity_b[:, :2]), dim=1)


# 3. 修正 penalize_ang_vel_xy_l2
def penalize_ang_vel_xy_l2(
        env: ManagerBasedRLEnv,
        asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """惩罚 XY 轴角速度 (晃动)"""
...
def penalize_lin_vel_z_l2(
    env: ManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """惩罚 Z 轴线速度 / Penalize linear velocity in Z axis."""
    asset: RigidObject = env.scene[asset_cfg.name]
    # 惩罚垂直方向的跳动
