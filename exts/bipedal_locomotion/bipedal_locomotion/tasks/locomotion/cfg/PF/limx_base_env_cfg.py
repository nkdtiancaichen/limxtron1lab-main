...
class PFSceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        ...
        terrain_type="generator",           # 地形类型：平面 / Terrain type: plane
        terrain_generator=MIXED_EXAM_TERRAINS_CFG,         # 不使用地形生成器 / No terrain generator used
...
@configclass
class RewardsCfg:
    """奖励项配置类 - 复杂地形越野版 (Rough Terrain Survival Mode)"""
    ...
    rew_lin_vel_xy = RewTerm(
        ...
        weight=1.5,  # <--- 从 10.0 降到 1.5 (只要大致方向对就行)
        params={"command_name": "base_velocity", "std": 0.5}  # <--- std 从 0.25 放宽到 0.5
    )
    ...
    pen_flat_orientation = RewTerm(
        ...
        weight=-0.2  # <--- 从 -5.0 降到 -0.2 (允许爬坡时身体倾斜)
    )

    pen_lin_vel_z = RewTerm(
        func=mdp.penalize_lin_vel_z_l2,
        weight=-0.5  # <--- 降低惩罚，允许上下台阶时的身体起伏
    )

    pen_ang_vel_xy = RewTerm(
        func=mdp.penalize_ang_vel_xy_l2,
        weight=-0.05  # <--- 降低惩罚，允许身体晃动来维持平衡
    )

    # 4. 高度与关节 (适应地形)
    pen_base_height = RewTerm(
        func=mdp.base_com_height,
        params={"target_height": 0.58},  # <--- 【关键】降低重心！0.78太高了，0.58方便屈膝爬楼
        weight=-1.0,
    )

    # [新增] 奖励脚在空中停留 (鼓励高抬腿/大步幅跨越障碍)
    rew_feet_air_time = RewTerm(
        ...
        params={
            ...
            "threshold": 0.3,  # 只有当命令速度 > 0.3 m/s 时才生效
        },
    )

    # 5. 动作平滑度 (允许爆发力)
    pen_action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.01)  # 降得很低
    pen_action_smoothness = RewTerm(func=mdp.ActionSmoothnessPenalty, weight=-0.01)

    # 6. 其他约束
    pen_joint_torque = RewTerm(func=mdp.joint_torques_l2, weight=-1.0e-5)  # 降低
    pen_joint_accel = RewTerm(func=mdp.joint_acc_l2, weight=-2.5e-7)
    pen_joint_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-5.0)  # 依然严防打限位

    # [删除] pen_feet_distance: 在复杂地形上，千万不要限制脚距！
    # [删除] pen_feet_regulation: 这个也容易干扰爬楼梯

    # 步态 (可以保留，作为辅助)
    test_gait_reward = RewTerm(
        func=mdp.GaitReward,
        weight=0.5,  # 降低权重，不要强求完美步态
        params={
