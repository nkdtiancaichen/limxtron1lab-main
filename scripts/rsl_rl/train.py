...
def dump_pickle(filename, data):
    """Replacement for isaaclab.utils.io.dump_pickle"""
...
def dump_yaml(filename, data):
    """Replacement for isaaclab.utils.io.dump_yaml"""
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    # 尝试将配置对象转换为字典，Isaac Lab配置通常有to_dict()方法
    if hasattr(data, "to_dict"):
        data_dict = data.to_dict()
    else:
        # 如果无法转换，尝试直接转储（可能需要根据实际情况调整）
        try:
            data_dict = vars(data)
        except:
            data_dict = data
